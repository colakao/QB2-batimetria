...
public class MouseOrbitOld : MonoBehaviour
{
	...
	private void CamDistance()
	{
		...
		{
			//var vect = transform.position - targetFocusTransform.position;
			//var offset = maxDistance - camDist;
			//vect.Normalize();
			//transform.position = offset * vect;
			//print("too far");
		}
	...
	private void MouseControls()
	{
		...
		if (Input.GetMouseButton(0))
		{
			...
			transform.RotateAround(currentCamPos, targetFocusTransform.up, Input.GetAxis("Mouse X") * orbitSpeed);
			float pitchAngle = Vector3.Angle(Vector3.up, transform.forward);
			...
			//var camVector = transform.position - targetFocusTransform.position;
